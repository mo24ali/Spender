...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let tl = gsap.timeline({ defaults: { ease: "power3.out" }, delay: 0.8 });
    ...
    tl.add(() => {
        let floatTl = gsap.timeline({ repeat: -1, yoyo: true });
    ...
    document.querySelectorAll('.carousel img').forEach(img => {
        img.addEventListener('mouseenter', () => {
            let scale = img.classList.contains('cards') ? 0.9 : img.classList.contains('mobile') ? 0.92 : 0.94;
            gsap.to(img, { scale, y: -5, duration: 0.3, ease: "power2.out", zIndex: 10 });
        });
        img.addEventListener('mouseleave', () => {
            let scale = img.classList.contains('cards') ? 0.85 : img.classList.contains('mobile') ? 0.88 : 0.9;
            let x = img.classList.contains('cards') ? 120 : img.classList.contains('mobile') ? -80 : -20;
            let y = img.classList.contains('cards') ? -10 : img.classList.contains('mobile') ? 5 : 0;
            let z = img.classList.contains('cards') ? 3 : img.classList.contains('mobile') ? 2 : 1;
